...
5. **符文自動配裝** — 選怪＋選套裝＋各洞主屬，引擎從符文池挑最佳解、湊套裝、算效率與符文加成合計；可勾選是否納入全帳號符文。**套裝需求彈性**：4件套可選「無」（不強制），2件套最多三組、可重複（例如元氣×2＝四洞元氣，套裝加成疊層）；4件套佔4洞時只開放1組2件套、選「無」則開放3組（超額洞數即時擋）；未被套裝佔用的「自由洞」不限套裝、依目標挑最佳符文。**配裝後最終體質**：以怪物體質(BASE_STATS，來源 SWARFARM 6★滿級) 套上該組符文的主/前/副屬(%＋固定值)＋套裝%加成，算出 HP/ATK/DEF/SPD/暴率/暴傷/抵抗/命中 的「基礎值 → 最終值」對照（未計入領導技/塔/神器/銘刻）。
   - **極大化屬性**：下拉選一個屬性（體力/攻擊/防禦/速度/暴率/暴傷/抵抗/命中），引擎改以該屬性最終值最大為目標挑符文；不指定則維持效率優先。
