...
// ---------- 符文庫 ----------
// 符文評級：效率門檻(社群標準) + 有效roll(只算「大六」副屬 SPD/暴率/暴傷/攻%/血%/防%；排除固定值與抵抗/集中)
...
const DUNGEONS=[
 {n:'巨人窟 GB12',sub:'符文/魔力石主要來源。核心：破防+多段+回能，可做全自動循環。',teams:[
   {tl:'穩定自動隊',us:[
     ...
     ['Bernard','回能+攻速buff','迅速/集中'],
     ['Sabrina','範圍破防','刀刃/集中'],
     ['Sigmarus','冰凍+輸出','刀刃/激怒'],
     ['Loren','多段輸出/減攻速','絕望/刀刃']]},
   {tl:'高速循環(需高速符)',us:[
     ['Bernard','回能','迅速/集中'],
     ['Galleon','破防+攻擊buff','迅速/集中'],
     ['Verdehile','爆擊回能','暴走/刀刃'],
     ['Lushen','斬殺(副手)','刀刃/激怒']]}]},
 {n:'巨龍巢穴 DB12',sub:'攻速/激怒/精準符文來源。需破防、抗擊暈、穩定輸出。',teams:[
   {tl:'通用隊',us:[
     ['Veromos','淨化+持續','絕望/意志'],
   ...
   {tl:'雙防破速刷',us:[
     ...
     ['Katarina','高輸出','暴走/刀刃'],
     ['Fran','免疫+奶','意志/迅速']]}]},
 {n:'死者之塔 NB12',sub:'激怒傷害/暴走/意志符文來源。怪物會反擊+回能,需破防與控場。',teams:[
   {tl:'通用隊',us:[
     ['Veromos','淨化+持續','絕望/意志'],
     ['Bernard','回能','迅速/集中'],
     ['Sabrina','破防+暈','刀刃/集中'],
     ['Sigmarus','冰凍','刀刃/激怒'],
     ['Loren','多段輸出','絕望/刀刃']]}]},
 {n:'鋼鐵要塞 SF10',sub:'免疫/專注/引導符文。BOSS會上保護,需高頻多段。',teams:[
   {tl:'多段隊',us:[
     ['Veromos','淨化+破防','絕望/意志'],
     ['Bernard','回能','迅速/集中'],
     ['Tesarion','多段+灼燒','暴走/刀刃'],
     ['Sath','破防+持續','絕望/意志']]}]},
 {n:'懲罰者地穴 PC10',sub:'反擊/破壞符文。BOSS高防,建議固傷/百分比傷害。',teams:[
   {tl:'固傷隊',us:[
     ['Veromos','淨化','絕望/意志'],
     ['Theomars','無視抗性輸出','暴走/刀刃'],
     ['Bernard','回能','迅速/集中'],
     ['Chasun','奶+回能','意志/迅速']]}]},
 {n:'元素地下城(火/風/水/光/暗 精髓)',sub:'進階素材(精髓)來源,難度低。任意屬性優勢隊即可穩過。',teams:[
   {tl:'通用清怪隊',us:[
     ['Lushen','範圍斬殺(光/暗皆有)','刀刃/激怒'],
     ['Veromos','淨化+持續','絕望/意志'],
     ['Bernard','回能','迅速/集中'],
     ['Colleen','奶+吸血','意志/吸血']]}]},
...
// ---------- 符文自動配裝 ----------
// 4件套(官方繁中)；選項改由實際符文資料產生，避免套裝名稱對不上(暴走/激怒/猛攻/集中…)
const FOUR4=['暴走','迅速','激怒','猛攻','絕望','吸血'];
const allSetNames=[...new Set(RUNES.map(r=>r.set).filter(Boolean))];
$('#fUnit').innerHTML=UNITS.filter(u=>u.s>=5).sort((a,b)=>a.n.localeCompare(b.n)).map(u=>`<option value="${u.id}">${u.n} (${u.at}${u.s}★)</option>`).join('');
$('#fSet4').innerHTML=allSetNames.filter(s=>FOUR4.includes(s)).map(s=>`<option>${s}</option>`).join('');
$('#fSet2').innerHTML=allSetNames.filter(s=>!FOUR4.includes(s)).map(s=>`<option>${s}</option>`).join('');
